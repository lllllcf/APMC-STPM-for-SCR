...
m = -1/3;
p1t = [];
Theta = [-40 -20 20 40]; % target location
for tt = 1:4
    theta = Theta(tt);
    ptot =[];
    p1 = [];
    ...
    %         t1i = (((i-1)*beta*z*sind(theta))/pi-1/6-a/3)/2*Tp;  
            s1i = t1i-m*Tp;
...
% set(gca, 'XTick', [0 1 2])
grid on
plot(alpha1, p1db(1,:),'--','LineWidth',1.5);
plot(alpha1, p1db(2,:),'-','LineWidth',1.5);
plot(alpha1, p1db(3,:),'-.','LineWidth',1.5);
plot(alpha1, p1db(4,:),'-.','LineWidth',1.5);
xlim([-90,90])
xlabel('\textbf{Angle (}$^\circ$\textbf{)}','Interpreter','Latex','FontSize',15);
ylabel('\textbf{Normalized Gain (dB)}','Interpreter','Latex','FontSize',15);

legend({'$\theta = -40^{\circ}$','$\theta = -20^{\circ}$',...
    '$\theta = 20^{\circ}$','$\theta = 40^{\circ}$'},'FontSize',12,'Interpreter','Latex')
