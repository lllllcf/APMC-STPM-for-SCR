...
%zz = lambda/8; % distance between two antennas in an element
zz = z/4;
d_t = zz*sind(theta)/c; % time delay modification
%d_t = 0; % time delay modification
   
for alpha = -89:1:89
    delta_t = z*sind(alpha)/c; % time delay of two signal
    delta_tt = delta_t/(1/fs); % transfer the time delay to the array index
    
       m_t = zz*sind(alpha)/c; % time delay of two signal
       m_tt = m_t/(1/fs);
%      m_tt = 0;
%m_tt = d_t;
...
Sig1_0 = sumIQRF_I'.*singlePulse(t11,tau1,Tp,t,fs,delta);
Sig1_90 = sumIQRF_Q'.*singlePulse(t11+Tp/4+d_t,tau1,Tp,t,fs,delta);
Sig1_180 = sumIQRF_I2'.*singlePulse(t11+Tp/2+d_t*2,tau1,Tp,t,fs,delta);
Sig1_270 = sumIQRF_Q2'.*singlePulse(t11-Tp/4+d_t*3,tau1,Tp,t,fs,delta);


ss = Sig1_0 + circshift(Sig1_90, floor(m_tt))+...
    circshift(Sig1_180, floor(m_tt)*2)+...
    circshift(Sig1_270, floor(m_tt)*3);
resSig = ss;
% resSig =Sig1_0+Sig1_90+Sig1_180+Sig1_270;
    for i=2:arrayNum
...
Sigi_0 = sumIQRF_I'.*singlePulse(t1i,tau1,Tp,t,fs,delta);
Sigi_90 = sumIQRF_Q'.*singlePulse(t1i+Tp/4+d_t,tau1,Tp,t,fs,delta);
Sigi_180 = sumIQRF_I2'.*singlePulse(t1i+Tp/2+d_t*2,tau1,Tp,t,fs,delta);
Sigi_270 = sumIQRF_Q2'.*singlePulse(t1i-Tp/4+d_t*3,tau1,Tp,t,fs,delta);

ss = Sigi_0 + circshift(Sigi_90, -floor(m_tt))+...
    circshift(Sigi_180, -floor(m_tt)*2)+...
    circshift(Sigi_270, -floor(m_tt)*3);
resSig = resSig + circshift(ss,-floor(delta_tt*(i-1)));

% resSig = resSig + circshift(Sigi_0+Sigi_90+Sigi_180+Sigi_270, -floor(delta_tt*(i-1)));
...
[f2,FFTsig]=SSBFFT(resSig,fs);
        ...
        p01 = find(f2>(fLO-0.3*fp),1);
        p02 = find(f2>(fLO+0.3*fp),1);
        ...
        pt1 = find(f2>(fLO-15*fp),1);
        pt2 = find(f2>(fLO+15*fp),1);
        
ptot = [ptot, sum(FFTsig(pt1:pt2).^2)]; 
p0 = [p0,sum(FFTsig(p01:p02).^2)];
p2 = [p2,sum(FFTsig(p21:p22).^2)]; 
...
legend({'\textbf{m=1}','\textbf{m=-1}','\textbf{m=3}','\textbf{m=5}','\textbf{m=-7}'},...
...
function res=singlePulse(t1,tau,Tp,totalTime,fs,delta)
    % repmat
    temp = 0:1/fs:delta;
    slope = temp/delta;
    len = length(slope);
    ...
    pp(floor((tau+delta)*fs)+1 : (floor(Tp*fs)+1)) = 0;
